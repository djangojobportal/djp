...
1. Close PyCharm
2. `make kill` or `docker-compose -p djp kill`
3. `make rm` or `docker-compose -p djp rm`
4. `make build`
5. `make build-devweb`
6. `make devweb`
7. start PyCharm
...
* **Name:** Django Server
* **Host:** 0.0.0.0
* **Port:** (use the http port specified in the docker-compose.yml file, e.g. 8080)`*` **Run browser** If checked, it will open the url after you click run. You should be able to access the running djp instance on 0.0.0.0:61202 (the port that mapped to 8080)
