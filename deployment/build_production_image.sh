#!/bin/bash
echo "Use this to build a production image of the djp-uwsgi"
...
then
    ...
    cd ..
	git archive --format=tar.gz $VERSION django_project > deployment/docker/djp.tar.gz
    cd -
    ...
    echo "ADD djp.tar.gz /home/web" >> docker/Dockerfile-prod
    echo "CMD [\"uwsgi\", \"--ini\", \"/uwsgi.conf\"]" >> docker/Dockerfile-prod
    # Now build the image and tag it
    cd docker
	docker build -t alisonmukoma/djp-uwsgi:latest -f Dockerfile-prod .
    docker tag alisonmukoma/djp-uwsgi:latest alisonmukoma/djp-uwsgi:$VERSION
    docker push alisonmukoma/djp-uwsgi:latest
    docker push alisonmukoma/djp-uwsgi:$VERSION
    cd -
	# Clean up
